...
const stripe = require('stripe')('sk_test_51PPYvZ00XtpcZ0UFYPD5yGt9KHjH6MzhsCCgj12EukQfaLBm61oADrQq0oiNa923MPhdwEUgXSJB2JYJtg5w9PKr00nqb4rhER'); // Replace 'your-secret-key' with your actual 
...
app.post('/api/openai', async (req, res) => {
    ...
    if (!conversationState[sessionId]) {
        ...
        `;
        const userValues = ['anonymous', 'anonymous', 'anonymous', conversationState[sessionId].conversationId];
                ...
                case 0:
                    ...
                    if (supportTicket && name && phoneNumber && email) {
                        ...
                        const updateUserDetailsQuery = `
                            ...
                            WHERE id = $4
                         `;
                        const updateUserDetailsValues = [name, email, phoneNumber, state.userId];
                        ...
                        console.log('Support ticket created:', ticketResult.rows[0].ticket_number);
                        await pool.query(
                            'INSERT INTO chatmessages (conversationid, message, role) VALUES ($1, $2, $3)',
                            [state.conversationId, prompt, 'ai']
                        );
...
app.get('/api/getMessages/:conversationId', async (req, res) => {
    ...
    try {
        const query = `SELECT * FROM chatmessages where conversationid ='`+String(conversationId)+`'`;
...
app.post('/api/products', upload.single('product_image'), async (req, res) => {
    const { product_name, product_price, product_category,generated_text } = req.body;
    ...
    try {
        const result = await pool.query(
            ...
            [login_id, product_name, product_price, product_category, product_image,generated_text]
        );
        res.json(result.rows[0]);
    } catch (error) {
...
app.get('/api/products',async (req,res)=>{
    try{
        const result = await pool.query(`SELECT * from storefront where sale is Null ORDER BY created_at DESC`);
        res.json(result.rows);
    }catch(error){
        res.status(500).json({error:'Internal server error'})
    }
...
app.get('/api/products/:loginid',async (req,res)=>{
    ...
    try{
        const result = await pool.query("SELECT * from storefront where login_id="+login_id+"ORDER BY created_at DESC");
        res.json(result.rows);
    }catch(error){
        res.status(500).json({error:'Internal server error'})
    }
...
app.get('/api/sale/products',async (req,res)=>{
    try{
        const result = await pool.query(`SELECT * from storefront where sale = true`);
        res.json(result.rows);
    }catch(error){
        res.status(500).json({error:'Internal server error'})
    }
...
app.get('/api/orders/:loginId', async (req, res) => {
    ...
    try {
        const result = await pool.query("SELECT orders.login_id as purchasedLoginId, orders.*, login.name as customer_name, orderItems.*,storefront.* FROM orders JOIN orderItems on orders.id = orderItems.order_id JOIN login ON orders.login_id = login.id JOIN storefront ON orderItems.product_id = storefront.id where storefront.login_id ="+loginID+" and orders.login_id !="+loginID+" ORDER BY orders.date desc");
        console.log(result.rows); 
        res.json(result.rows);
    } catch (error) {
        console.error('Error fetching orders:', error);
...
app.post('/api/orders', async (req, res) => {
    ...
    const {login_id, order_amount,product_id,fullName,address,phoneNumber,city,zipcode,country} = req.body;
    ...
    try {
        const result = await pool.query(
            ...
            [login_id, new Date(), order_amount,fullName,address,phoneNumber,city,zipcode,country]
        );
        console.log("order result"+result.rows[0])
        //res.json(result.rows[0]);

        const orderItemsResult = await pool.query('INSERT INTO orderItems (order_id,product_id,product_price) VALUES ($1,$2,$3) RETURNING *',[result.rows[0].id,product_id,order_amount])
        res.json(result.rows[0])
...
app.post('/api/make-call', async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query('SELECT phone_number FROM orders WHERE id = $1', [orderId]);
      ...
      client.calls
        .create({
          twiml: '<Response><Say>Your order has been placed successfully!</Say></Response>',
          to: phoneNumber,
          from: '+17579942304',  
        })
